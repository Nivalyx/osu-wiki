...
In osu! gameplay, **converts** (or *converted beatmaps*) are [beatmaps](/wiki/Beatmap) from a specific game mode that are "converted" to be playable in another game mode for which they are not originally created. All converts are automatically generated by the game based on certain parameters present on the base beatmap.
