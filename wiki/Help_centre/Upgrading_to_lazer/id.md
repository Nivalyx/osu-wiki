...
Kamu dapat mengatur pilihan `Mode tampilan skor` ke `Klasik` untuk menampilkan model skor yang "eksplosif" seperti pada stable. Walaupun tidak persis sama, model ini memiliki perilaku yang mirip dengan model stable dan dapat digunakan pada mode permainan mana pun.
