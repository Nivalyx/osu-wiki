...
Kamu dapat mengatur pilihan `Mode tampilan skor` ke `Klasik` untuk mengaktifkan model skor yang "eksplosif" seperti pada stable. Walaupun tidak persis sama, model ini memiliki perilaku yang serupa dengan stable dan dapat diterapkan di mana pun.
...
Setelah melewati ambang batas tertentu, FPS yang berlebihan hanya akan menghambur-hamburkan sumber daya yang ada. Untuk mencegah hal ini, lazer menggunakan berbagai teknologi mutakhir agar jeda input yang ada dapat diminimalkan bahkan pada FPS yang rendah sekalipun.
...
Walaupun pada sebagian besar hardware modern performa lazer lebih baik dari stable, terdapat beberapa konfigurasi hardware di mana kinerja lazer masih terhambat. Dalam waktu dekat, kami berencana untuk memecahkan masalah ini dengan menghadirkan dukungan DirectX (atau yang dikenal pada stable dengan istilah "compatibility mode") dan Vulkan, yang memiliki dukungan lintas hardware yang lebih baik dibanding OpenGL. Setelah keduanya didukung, chipset Intel yang terintegrasi
