...
| Object's end is offscreen! | The end of the object is not fully onscreen on 4:3 resolution. | Move the position of the object's end. | AiMod doesn't always detect the upper bounds of the playfield properly, so make sure to verify whether the object in question is actually offscreen or not from time to time. |
