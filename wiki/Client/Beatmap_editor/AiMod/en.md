...
| <!-- Potential removal? Wasn't able to make an actual object (not including slider ends) go offscreen as editor forced it back in again -->Object isn't snapped! | The specified object isn't snapped onto the timeline. | Snap the If you have made changes to the song's timing beforehand, you may need to resnap all the notes (you can do this manually or click Timing » Resnap all notes (if you do this, you will need to check to see if any notes were placed in the wrong spot!)). | This warning can be safely ignored if the object is intended to be snapped to a [divisor](http://osu.ppy.sh/wiki/en/Client/Beatmap_editor/Beat_Snap_Divisor) that's not currently supported by the editor (e.g. 1/10). |
