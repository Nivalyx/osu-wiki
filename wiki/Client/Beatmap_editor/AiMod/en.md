...
| Object's end is offscreen! | The object's end is not onscreen (whether fully or partially) on 4:3 resolution. | Remove the object or move the position of the object's end. | AiMod doesn't always detect the upper bounds of the playfield properly, so make sure to verify whether the object's end in question is actually offscreen or not from time to time. |
| Object is offscreen! | The object is not onscreen (whether fully or partially) on 4:3 resolution. | Remove or move the object. | AiMod doesn't always detect the upper bounds of the playfield properly, so make sure to verify whether the object in question is actually offscreen or not from time to time. |
