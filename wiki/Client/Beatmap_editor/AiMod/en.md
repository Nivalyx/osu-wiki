...
| Message | Explanation | Solution | Notes |
| :-- | :-- | :-- |  :-- |
| These two objects are less than 10ms apart! | Two objects are placed very close to each other timeline-wise, requiring the player to click both objects almost simultaneously. | Find the two hit objects either move or delete one of them. | |
...
| This hold note is less than 10ms long! | A hold note is , rendering the map impossible to SS. | Increase length of the hold note or remove it. | ![](/wiki/shared/mode/mania.png) osu!mania maps only |
