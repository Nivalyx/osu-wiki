...
| This mapset cannot have an Insane diff without a Hard diff. | *Self-explanatory* | Create a Hard difficulty for the set. | This warning can safely be ignored if the map's drain time is long enough to the point that the map can be ranked without a Hard difficulty as per the [ranking criteria's spread requirement](wiki/Ranking_Criteria#beatmap). |
