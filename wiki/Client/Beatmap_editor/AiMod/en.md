...
| Audio file conflicts with {0} diff. | The .mp3 file used in the map differs between one difficulty and another. | Make sure that the .mp3 file in the map is the same across all difficulties. | |
| Countdown conflicts with {0} diff. | The `Enable countdown` option in the Design tab of [Song Setup](/wiki/Client/Beatmap_editor/Song_Setup) differs between one difficulty and another. | Make sure that the `Enable countdown` setting is consistent across all difficulties. |
