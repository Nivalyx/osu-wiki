...
To make that chart more understandable and state more precisely what is meant, this guide will now explain a bit of the relevant music theory behind this, and give examples of how it translates into osu!. For this reason, the following three songs will be used as a reference throughout this guide:
...
As an use-case example in osu!, consider the song Leonard Bernstein's "America". In this song, everything is timed in `3/4-signature`, meaning that every measure is divided into three beats by white ticks. Setting the BeatSnapDivisor to `1/2` will make every beat to be split into two by the red ticks in the timeline (as in all the red ticks will be representing an eighth-note `♪`). This would work perfectly fine for a `3/4`-song, but upon listening closely the song actually reveals something more: While the overall tempo stays (approximately) the same, the rhythmical grouping of `♪`s switches almost every measure. 
